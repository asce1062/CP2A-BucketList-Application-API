...
class BucketListAPI(Resource):
    ...
    def get(self, id=None):
        ...
        if id:
            this_bucketlist = Bucketlist.query.filter_by(
                bucket_id=id, created_by=g.user.user_id).first()
            ...
            if query:
                bucketlists = Bucketlist.query.filter(
                    Bucketlist.bucket_name.like('%' + query + '%'),
            ...
            return {
                ...
                'bucketlists': marshal(bucketlists.items, bucketlist_fields)
            }, 200
    ...
    def post(self):
        ...
        new_bucketlist = Bucketlist(
            bucket_name=name, created_by=g.user.user_id)
        db.session.add(new_bucketlist)
        db.session.commit()

        return {
            'message': 'New bucketlist created successfully',
            'bucketlist': marshal(new_bucketlist, bucket_item_fields)
        }, 201
    ...
    def put(self, id):
        ...
        update_bucketlist = Bucketlist.query.filter_by(bucket_id=id).first()
        update_bucketlist.bucket_name = name
        db.session.commit()

        return {
            'message': 'Update was successfull',
            'bucketlist': marshal(update_bucketlist, bucket_item_fields)
        }, 201
...
class BucketItemAPI(Resource):
    ...
    def __init__(self):
        ...
        def get(self, itemid=None):
            ...
            if itemid:
                ...
                page = args['page']
                if query:
                    items = BucketItem.query.filter(
                        BucketItem.item_name.like('%' + query + '%')).paginate(page, limit, False)
                else:
                ...
                return {
                    ...
                    'items': marshal(items.items, bucket_item_fields)
                }, 200
    ...
    def post(self, bucketlist_id):
        ...
        bucketlist_exists = Bucketlist.query.filter_by(
            bucket_id=bucketlist_id, created_by=g.user.user_id).first()
        ...
        return {
            ...
            'bucketlist': marshal(new_item, bucket_item_fields)
        }, 201
    ...
    def put(self, bucketlist_id, item_id):
        ...
        bucketlist_exists = Bucketlist.query.filter_by(
            bucket_id=bucketlist_id).first()

        if bucketlist_exists:
            item_exists = BucketItem.query.filter_by(
